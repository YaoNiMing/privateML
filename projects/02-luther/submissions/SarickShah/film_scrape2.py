...
def solver(list_of_variables, mpaaRating): 
    ...
    for category in list_of_variables: 
        try: 
            if category == 'Rotten Tomatoes':  
                ...
                #rating1, rating2 = map(lambda x: re.search('\d+%',x).group(0), rating)
    ...
    print('passing full iteration of solver for one movie')
...
def txt_link_downloader(html_link):
    ...
    counter = 0
    for index,i in enumerate(xrange(600,len(batch),6)):
        ...
        #second_page = map(lambda x:','.join(x),second_page)
